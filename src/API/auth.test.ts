...
import {apiAuth, AuthTypeReturn, EmailType, LoginType} from "./authAPI";
...
it('authApiAuth', async () => {
    try {
        ...
        const {response} = err as AxiosError<AuthTypeReturn<string>>
...
it('authArentAuth', async () => {
    try {
        const {data} = await apiAuth.registration({...user, email: `${Math.random().toString(36).slice(2)}@gmail.com`})
...
it('logintest', async () => {
    await expect(apiAuth.registration({
        ...user,
        email: `${Math.random().toString(36).slice(2)}@gmail.com`
    })).resolves.toBeTruthy()
})
it('SendEmailApiResolve', async () => {
    await expect(apiAuth.sendEmail({email: user.email, name: 'Exoool', verify: 'test'}))
})
