...
instance.interceptors.request.use((config: AxiosRequestConfig) => {
    if (config.headers) config.headers.Authorization = `Bearer ${window.localStorage.getItem('token')}`
    return config
})

export const wordApi = {
    getWords(count: number): AxiosPromise<ProjectTypeReturn<ProfileType>> {
        return instance.get<ProjectTypeReturn<ProfileType>>(`words/?count=${count}`)
    },
    ...
    findWords(word: string): AxiosPromise<ProjectTypeReturn<Array<WordType>>> {
        return instance.post<ProjectTypeReturn<Array<WordType>>>(`word-find?word=${word}`)
    },
