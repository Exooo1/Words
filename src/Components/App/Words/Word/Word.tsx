...
export const Word: React.FC<WordType> = ({id, word, description, deleteWord, added, translate}) => {
    ...
    return isEdit ? <div className='container_word' key={word}>
        <input type="text" value={wor} onChange={(e) => setWor(e.target.value)}/>
        <input type="text" value={tran} onChange={(e) => setTran(e.target.value)}/>
        <input type="text" value={descrip || 'your description'} onChange={(e) => setDescrip(e.target.value)}/>
        <p className={'container_word_added'}>{added}</p>
        <img onClick={acceptChange} src="https://cdn-icons-png.flaticon.com/512/2258/2258597.png" alt=""/>
        <button onClick={deleteWord}>X</button>
    </div> : <div className='container_word' key={word}>
        ...
        <p className={'container_word_added'}>{added}</p>
        <img src="https://cdn-icons-png.flaticon.com/512/2356/2356811.png" alt="" onClick={() => setIsEdit(true)}/>
        <button onClick={deleteWord}>X</button>
    </div>
}
