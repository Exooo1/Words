...
export const useForm = (): FormType => {
    ...
    const createAccount = () => {
        if (name.length < 1) return dispatch(addHint({article: 'Where is your name?', status: 'error'}))
        if (surname.length < 1) return dispatch(addHint({article: 'Where is you surname?', status: 'error'}))
        if (password.length! < 6) return dispatch(addHint({article: 'Password incorrect', status: 'error'}))
